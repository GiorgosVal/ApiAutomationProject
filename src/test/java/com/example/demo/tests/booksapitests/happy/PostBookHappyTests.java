...
public class PostBookHappyTests extends TestsBase {

    @Autowired
    public BooksApiBO booksApiBo;

    @Test(testName = "POST book: Verify that a new Book is accepted and response body is correct",
            dataProvider = "validBooks", dataProviderClass = BooksDataProvider.class,
            groups = {REGRESSION, SMOKE, HAPPY_PATH, BOOKS_API, BOOKS_API_POST})
    @Description("POST book: Verify that a new Book is accepted and response body is correct")
    public void testStep1(BookDTO bookToCreate) {
        booksApiBo.postAndVerifyResponseCodeJsonSchemaAndBody(bookToCreate, BookDTO.class);
    }
}
